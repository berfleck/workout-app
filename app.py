...
with st.sidebar:
    ...
    # Botão gerar oculto na sidebar (capturado pelo JS do floating)
    gerar = st.button("▶ Gerar treino", type="primary", use_container_width=True, key="gerar_sidebar")

# Botão floating — clica no botão oculto da sidebar via JS
st.markdown("""
<button class="floating-gerar" onclick="
    const btns = window.parent.document.querySelectorAll('button[kind=primary]');
    for (const b of btns) {
        if (b.innerText.includes('Gerar')) { b.click(); break; }
    }
">▶ Gerar treino</button>
""", unsafe_allow_html=True)
...
# ===========================================================================
with tab_treino:
    # Unifica a fonte de disparo (sidebar — acionado pelo botão floating ou direto)
    disparar_geracao = gerar
